...
class AnggotaController extends Controller
{
    public function rekapTabungan(Request $request)
    {
        ...
        $rekening = Rekening::where('anggota', $anggota)->first();
        // Query untuk data tabungan
        $tabungan = Tabungan::with(['rekening.kategori', 'rekening.tellers.profile'])->where('rekening_id', $rekening->id_rekening)
        ...
        return view('anggota.rekap.tabungan-rekap', $data);
    }
    ...
    public function rekapPinjaman(Request $request)
    {
        ...
        $pinjaman = Pinjaman::with(['rekening.kategori', 'rekening.tellers.profile'])->where('rekening_id', $rekening->id_rekening)
            ->when($tanggal, function ($query, $tanggal) {
                return $query->whereDate('created_at', $tanggal);
            })
            ->when($cabang_id, function ($query, $cabang_id) {
        ...
        return view('anggota.rekap.pinjaman-rekap', $data);
    }

    public function rekapPegadaian(Request $request)
    {
        // Ambil filter dari request
        $tanggal = $request->input('tanggal');
        $cabang_id = $request->input('cabang_id');
        ...
        $pegadaian = Pegadaian::with(['rekening.kategori', 'rekening.tellers.profile'])->where('rekening_id', $rekening->id_rekening)
            ->when($tanggal, function ($query, $tanggal) {
                return $query->whereDate('created_at', $tanggal);
            })
            ->when($cabang_id, function ($query, $cabang_id) {
        ...
        return view('anggota.rekap.pegadaian-rekap', $data);
    }
}
